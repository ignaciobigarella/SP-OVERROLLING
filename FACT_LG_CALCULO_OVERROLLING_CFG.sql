...
DELETE FROM [GES_Siderar].DBO.[fact_lg_calculo_overrolling_cfg]
...
select 'roll_chance_slabs_sobrantes', 19, 'ROLL CHANCE', 'ETP-3BLC.003', '##fact_lg_calculo_overrolling_RC_BLC', '''33''', '''ETP-3BLC.003''' , 'insert into @tabla_voces', 'select material_key', 'from ##fact_lg_calculo_overrolling_posible_ovr ovr', 'where ovr.cod_tipo_movimiento in (@cod_tipo_movimiento)', 'and (ovr.pro_norma_desc in (@pro_norma_desc))'
...
set identity_insert [GES_Siderar].DBO.[fact_lg_calculo_overrolling_cfg] ON

insert into [GES_Siderar].DBO.[fact_lg_calculo_overrolling_cfg]

(id, prioridad_voces, codigo, voces, subvoces, tabla_voces, cod_tipo_movimiento, pro_norma_desc, cod_cliente, valor_posterior,
pro_grado_desc, pro_subnorma_desc, tipo, clase_cod, pro_cod, cod_motivo_pedido, cod_tipo_ofa,
espesor_valor, ancho_valor, cod_grado_acero, marca_logica_calculo, planta, largo_valor,
cod_tipo_calidad, cod_linea_imputacion, ofa_necesidad, ofadoc_ofapos, valor_anterior,
desc_defecto, cod_linea, desc_causa_defecto, cod_clase_doc, calidad_cod, gale, observaciones,
cond_update, cond_insert, cond_select, cond_from, cond_join, cond_where, cond_and_or, cond_groupby, orden)

select * from ##fact_lg_calculo_overrolling_definicion

set identity_insert [GES_Siderar].DBO.[fact_lg_calculo_overrolling_cfg] OFF

select * from [GES_Siderar].DBO.[fact_lg_calculo_overrolling_cfg]
order by orden
