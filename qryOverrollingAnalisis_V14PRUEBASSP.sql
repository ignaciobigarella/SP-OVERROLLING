			...
			SET @FechaIni = '2022-01-01 00:00:00.000'
			SET @FechaFin = '2022-06-30 23:59:59.999'
			...
			select t1.*
			into ##fact_lg_calculo_overrolling_RECUEBXCHT
			from ##fact_lg_calculo_overrolling_tmp t1 with(nolock)
			inner join ##fact_lg_calculo_overrolling_tmp t2 on t1.material_key = t2.material_key and
			...
			insert into ##fact_lg_calculo_overrolling_recuperos_ovr
			select * from ##fact_lg_calculo_overrolling_RECUEBXCHT recu with(nolock)

			-- Elimino casos OVE - EBX de tmp
			delete t from ##fact_lg_calculo_overrolling_tmp t 
			inner join ##fact_lg_calculo_overrolling_RECUEBXCHT r with(nolock) on r.material_key = t.material_key
			...
			-- CONSULTA FINAL
			select ovr.cod_clase_doc tipo_pedido, 
			ovr.id_movimiento, ovr.fact_lg_movimientos_cab_key, ovr.material_key, ovr.nro_bulto, ovr.Cod_Tipo_Movimiento, ovr.Desc_Tipo_Movimiento,
			ovr.Variable_Cod, ovr.valor_anterior, ovr.valor_posterior, ovr.Fecha_Movimiento, ovr.cod_cliente, 
			ovr.pro_norma_desc, ovr.pro_grado_desc, ovr.pro_subnorma_desc, ovr.Clase_cod, ovr.espesor_valor, ovr.ancho_valor, ovr.largo_valor, ovr.peso_neto,
			ovr.cod_grado_acero, ovr.desc_tipo_calidad, ovr.cod_tipo_ofa, ovr.desc_tipo_calidad, ovr.Planta, ovr.tipo, ovr.ofa_documento, ovr.ofa_posicion, 
			ovr.ofa_necesidad, ovr.marca_logica_calculo, ovr.cod_linea cod_linea_origen, ovr.cod_linea_imputacion, ovr.desc_defecto, ovr.desc_causa_defecto,
			ovr.pro_cod, ovr.cod_motivo_pedido, ovr.calidad_cod, ovr.cod_calidad,
			ovr.gale, voz, subvoz from ##fact_lg_calculo_overrolling_posible_ovr ovr with(nolock)
			where convert(varchar(8),ovr.fecha_movimiento,112) between '20220101' and '20220630'
			and subvoz = 'CHILE ETP-3ESP.003'
			order by nro_bulto